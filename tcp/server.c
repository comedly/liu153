...
int main()
{
	int sock = socket(AF_INET,SOCK_STREAM,0);//创建并返回一个通信端点描述符
	if(sock < 0)
	{
	...
	if(bind(sock,(struct sockaddr*)&server_socket,sizeof(struct sockaddr_in)) < 0)//分配一个名字到套接字，即绑定sock的源地址
	{
	...
	if(listen(sock,_BACKLOG_) < 0)//使sock成为一个监听描述符,用于接受传入的连接请求
	{
	...
	{
		socklen_t len = 0;
		int client_sock = accept(sock,(struct sockaddr*)&client_socket,&len);//阻塞等待客户端连接
